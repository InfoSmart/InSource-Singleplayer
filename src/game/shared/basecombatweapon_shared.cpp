...
void CBaseCombatWeapon::Activate()
{
	BaseClass::Activate();
...
//-----------------------------------------------------------------------------
const unsigned char *CBaseCombatWeapon::GetEncryptionKey() 
{ 
...
// Purpose:
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::Precache()
{
#if defined( CLIENT_DLL )
...
//-----------------------------------------------------------------------------
const FileWeaponInfo_t &CBaseCombatWeapon::GetWpnData() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *CBaseCombatWeapon::GetWorldModel() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *CBaseCombatWeapon::GetAnimPrefix() const
{
...
// Output : char const
//-----------------------------------------------------------------------------
const char *CBaseCombatWeapon::GetPrintName() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
int CBaseCombatWeapon::GetMaxClip2() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
int CBaseCombatWeapon::GetDefaultClip1() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
int CBaseCombatWeapon::GetDefaultClip2() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CBaseCombatWeapon::UsesClipsForAmmo1() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CBaseCombatWeapon::UsesClipsForAmmo2() const
{
...
//-----------------------------------------------------------------------------
bool CBaseCombatWeapon::AllowsAutoSwitchFrom() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
int CBaseCombatWeapon::GetWeaponFlags() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
int CBaseCombatWeapon::GetSlot() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
int CBaseCombatWeapon::GetPosition() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
const char *CBaseCombatWeapon::GetName() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
CHudTexture const *CBaseCombatWeapon::GetSpriteActive() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
CHudTexture const *CBaseCombatWeapon::GetSpriteInactive() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
CHudTexture const *CBaseCombatWeapon::GetSpriteAmmo() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
CHudTexture const *CBaseCombatWeapon::GetSpriteAmmo2() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
CHudTexture const *CBaseCombatWeapon::GetSpriteCrosshair() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
CHudTexture const *CBaseCombatWeapon::GetSpriteAutoaim() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
CHudTexture const *CBaseCombatWeapon::GetSpriteZoomedCrosshair() const
{
...
// Purpose: 
//-----------------------------------------------------------------------------
CHudTexture const *CBaseCombatWeapon::GetSpriteZoomedAutoaim() const
{
...
{
	if ( gpGlobals->curtime > m_flTimeWeaponIdle && m_flTimeWeaponIdle != 0 )
		return true;
...
void CBaseCombatWeapon::SetPickupTouch()
{
#if !defined( CLIENT_DLL )
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CBaseCombatWeapon::IsWeaponVisible()
{
	CBaseViewModel *vm = NULL;
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
	if ( pOwner )
...
//-----------------------------------------------------------------------------
bool CBaseCombatWeapon::ReloadOrSwitchWeapons()
{
	CBasePlayer *pOwner = ToBasePlayer(GetOwner());
	Assert(pOwner);
	...
	// Al parecer nos hemos quedado sin munición, sacar la siguiente arma.
	if (!HasAnyAmmo() && m_flNextPrimaryAttack < gpGlobals->curtime && m_flNextSecondaryAttack < gpGlobals->curtime)
	{
		// El arma no es usable, cambiarla otra vez...
	...
	{
		// El arma es usable, recargar si esta vacio y si así esta configurada.
		if (UsesClipsForAmmo1() && 
			 (m_iClip1 == 0) && 
		...
		{
			// InSource
			// No recargar automáticamente o tomar más tiempo si estamos en modo "principalmente manejando armas"
			ConVarRef in_beginner_weapon("in_beginner_weapon");

			if(in_beginner_weapon.GetBool() == false || random->RandomInt(0, 500) == 5)
			{
				if (Reload())
					return true;
			}
...
Activity CBaseCombatWeapon::GetDrawActivity()
{
	return ACT_VM_DRAW;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::HideThink()
{
	// Only hide if we're still the active weapon. If we're not the active weapon
	if ( GetOwner() && GetOwner()->GetActiveWeapon() == this )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::ItemPreFrame()
{
...
// WEAPON BEHAVIOUR
//====================================================================================
void CBaseCombatWeapon::ItemPostFrame()
{
	CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
	...
	{
		if(GetReloadTime() == 0)
			SetReloadTime(gpGlobals->curtime + 1);
...
/*
	Función - Apocalypse
	Recargar en estado de inactividad.
*/
void CBaseCombatWeapon::ReloadInIdle()
{
	ConVar *in_automatic_reload = cvar->FindVar("in_automatic_reload");

	if(in_automatic_reload->GetInt() == 0)
		return;

	if(!ReloadTimeElapsed() || !UsesClipsForAmmo1() || m_iClip1 <= 0)
		return;
...
*/
void CBaseCombatWeapon::SetReloadTime(float time)
{
	m_flReloadTime = time;
}
...
*/
float CBaseCombatWeapon::GetReloadTime()
{
...
/*
	Función - Apocalypse
	Obtener si el arma esta descansando para la recarga.
*/
bool CBaseCombatWeapon::ReloadTimeElapsed()
{
	if ( gpGlobals->curtime > m_flReloadTime && m_flReloadTime != 0 )
		return true;
...
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::ItemBusyFrame()
{
}
...
// Output :
//-----------------------------------------------------------------------------
int CBaseCombatWeapon::GetBulletType()
{
	return 0;
}
...
// Output :
//-----------------------------------------------------------------------------
const Vector& CBaseCombatWeapon::GetBulletSpread()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CBaseCombatWeapon::Reload()
{
	return DefaultReload( GetMaxClip1(), GetMaxClip2(), ACT_VM_RELOAD );
}

//=========================================================
void CBaseCombatWeapon::WeaponIdle()
{
...
//=========================================================
Activity CBaseCombatWeapon::GetPrimaryAttackActivity()
{
	return ACT_VM_PRIMARYATTACK;
}

//=========================================================
Activity CBaseCombatWeapon::GetSecondaryAttackActivity()
{
...
//-----------------------------------------------------------------------------
char *CBaseCombatWeapon::GetDeathNoticeName()
{
#if !defined( CLIENT_DLL )
...
// WEAPON RELOAD TYPES
//====================================================================================
void CBaseCombatWeapon::CheckReload()
{
	if ( m_bReloadsSingly )
	{
		CBasePlayer *pOwner = ToBasePlayer( GetOwner() );
...
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::AbortReload()
{
#ifdef CLIENT_DLL
...
//-----------------------------------------------------------------------------
void CBaseCombatWeapon::MaintainIdealActivity()
{
...
BEGIN_PREDICTION_DATA( CBaseCombatWeapon )
	...
	// Apocalypse - Tiempo de recarga automatica.
	DEFINE_PRED_FIELD_TOL( m_flReloadTime, FIELD_FLOAT, FTYPEDESC_INSENDTABLE, TD_MSECTOLERANCE ),
	...
	DEFINE_PRED_FIELD( m_flReloadTime, FIELD_FLOAT, FTYPEDESC_INSENDTABLE ), // Apocalypse - Tiempo de recarga automatica
...
BEGIN_DATADESC( CBaseCombatWeapon )
	...
	DEFINE_FIELD( m_flReloadTime, FIELD_TIME ), // Apocalypse - Recarga automatica
...
BEGIN_NETWORK_TABLE_NOBASE( CBaseCombatWeapon, DT_LocalActiveWeaponData )
	...
	SendPropTime( SENDINFO( m_flTimeWeaponIdle ) ), // Apocalypse - Tiempo de recarga automatica.
	...
	RecvPropTime( RECVINFO( m_flReloadTime ) ), // Apocalypse - Tiempo de recarga automatica.
#endif
END_NETWORK_TABLE()
