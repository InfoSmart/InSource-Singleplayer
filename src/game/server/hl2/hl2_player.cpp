...
ConVar sv_infinite_aux_power				("sv_infinite_aux_power", "0", FCVAR_CHEAT, "Energía del traje inifinita");
...
//=========================================================
// Tabla de daño de objetos con física.
//=========================================================
...
//=========================================================
//	CLogicPlayerProx
//	Usado para transmitir eventos outputs/inputs del
//	jugador al mundo y viceversa.
//=========================================================
...
static ConCommand toggle_zoom("toggle_zoom", CC_ToggleZoom, "Intercambiar modo de Zoom");
...
//=========================================================
//	CC_ToggleZoom
//	Usar o dejar de usar el agachado
//=========================================================
void CC_ToggleDuck()
{
	CBasePlayer* pPlayer = UTIL_GetCommandClient();
...
//=========================================================
// Configuración de la energía del traje
//=========================================================
...
//=========================================================
// CheckSuitZoom
// Realizar zoom con traje de protección
//=========================================================
...
//=========================================================
//	EquipSuit()
//	Equipar y activar el traje de protección.
//---------------------------------------------------------
// - bPlayEffects: ¿Ejecutar animación de admiración de las manos?
//=========================================================
void CHL2_Player::EquipSuit(bool bPlayEffects)
{
	...
	/*
		Iván: Comentado temporalmente ¡Es molesto para trabajar!

	SetSuitUpdate("!HEV_A0", false, CSUITNOREPEAT);
	SetSuitUpdate("!HEV_A3", false, CSUITNOREPEAT);
	SetSuitUpdate("!HEV_A4", false, CSUITNOREPEAT);
	SetSuitUpdate("!HEV_A5", false, CSUITNOREPEAT);
	SetSuitUpdate("!HEV_A6", false, CSUITNOREPEAT);
	SetSuitUpdate("!HEV_A7", false, CSUITNOREPEAT);
	SetSuitUpdate("!HEV_A10", false, CSUITNOREPEAT);
	*/
}

//=========================================================
// RemoveSuit()
// Remover el traje de protección
//=========================================================
void CHL2_Player::RemoveSuit()
{
...
// Empezar a correr
//=========================================================
void CHL2_Player::StartSprinting()
{
	// No correr a menos que haya una cantidad minima de energía en el traje.
...
// Ya no correr
//=========================================================
void CHL2_Player::StopSprinting()
{
...
//=========================================================
// EnableSprint()
// Activar o desactivar la capacidad de correr.
//=========================================================
...
//=========================================================
// IsZooming()
// ¿Estamos haciendo zoom?
//=========================================================
bool CHL2_Player::IsZooming()
{
...
{
	const float MAX_NORMAL_BATTERY = 300;
...
//=========================================================-------------------------
//=========================================================-------------------------
#define FLASHLIGHT_RANGE	Square(500)
...
{
	// La política "gordon_invulnerable" esta activada, hacer al jugador invulnerable.
	if (GlobalEntity_GetState("gordon_invulnerable") == GLOBAL_ON)
		return 0;

	// Ignorar el daño por caida.
	...
	{
		// Al parecer estamos sumergidos en el agua.
		if(GetWaterLevel() > 2)
		{
			// Sin daños por explosión debajo del agua.
	...
	if (info.GetDamage() > 0.0f)
	{
		...
		// Notificar a mis amigos del ataque. ¿Npc?
		if (info.GetAttacker())
			NotifyFriendsOfDamage(info.GetAttacker());
	}
	
	// Modificar el daño recibido del jugador dependiendo de su habilidad.
	CTakeDamageInfo playerDamage = info;

	// Lo de arriba.
	bool bAdjustForSkillLevel = true;

	// Evitar la modificación del daño por habilidad si el jugador se ha hecho daño el mismo. 
	// ¿Pero que tan tonto puedes ser?
	if(info.GetDamageType() == DMG_GENERIC && info.GetAttacker() == this && info.GetInflictor() == this)
		bAdjustForSkillLevel = false;

	// Proteger al usuario si esta en un vehiculo.
...
void CHL2_Player::UpdateClientData( void )
{
	...
	if ( Flashlight_UseLegacyVersion() != false )
	{
		...
		/*
			Apocalypse - Linterna - Uso infinito.
