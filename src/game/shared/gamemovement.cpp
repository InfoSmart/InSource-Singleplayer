...
/*
	InSource - Oscilación de cámara
	Variables
*/
ConVar in_viewbob_enabled("in_viewbob_enabled", "1", 0, "Oscilación de cámara", true, 0, true, 1);
ConVar in_viewbob_timer("in_viewbob_timer", "10", 0, "Velocidad de la oscilación");
ConVar in_viewbob_scale("in_viewbob_scale", "0.05", 0, "Magnitud de la oscilación");
...
{
	/*
		Apocalypse - Movimientos - Oscilación de cabeza
	*/
	if ( in_viewbob_enabled.GetInt() == 1 && !engine->IsPaused() )
	{
		float xoffset = sin( gpGlobals->curtime * in_viewbob_timer.GetFloat() ) * player->GetAbsVelocity().Length() * in_viewbob_scale.GetFloat() / 100;
		float yoffset = sin( 2 * gpGlobals->curtime * in_viewbob_timer.GetFloat() ) * player->GetAbsVelocity().Length() * in_viewbob_scale.GetFloat() / 400;
