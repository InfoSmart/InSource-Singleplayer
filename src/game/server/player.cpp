...
//----------------------------------------------------
// Sombra de la física del jugador.
//----------------------------------------------------
...
ConVar  player_debug_print_damage("player_debug_print_damage", "0", FCVAR_CHEAT, "Activa o desactiva el envio de mensajes de daño del usuario a la consola.");

CBaseEntity	*g_pLastSpawn = NULL;

/*----------------------------------------------------
	CC_GiveCurrentAmmo()
	Darle al usuario munición del arma actual.
----------------------------------------------------*/
...
static ConCommand givecurrentammo("givecurrentammo", CC_GiveCurrentAmmo, "Obtener munición para el arma actual..\n", FCVAR_CHEAT);
...
/*---------------------------------------------------
	UpdateTransmitState()
	Actualizar transmisión de estado.
	NOTA: No se sabe que hace exactamente...
---------------------------------------------------*/
...
/*---------------------------------------------------
	...
	Calcular si la entidad desea una compensación de Lag.
	NOTA: Falta de investigación.
---------------------------------------------------*/
bool CBasePlayer::WantsLagCompensationOnEntity( const CBasePlayer *pPlayer, const CUserCmd *pCmd, const CBitVec<MAX_EDICTS> *pEntityTransmitBits ) const
{
	...
	// Calcular la máxima distancia de compensación.
...
void CBasePlayer::TraceAttack(const CTakeDamageInfo &inputInfo, const Vector &vecDir, trace_t *ptr)
{
	...
	#ifdef HL2_EPISODIC
		// Verificar si el daño produce sangrado.
...
/*---------------------------------------------------
	DamageEffect()
	Muestra el efecto en el indicar según sea el daño.
---------------------------------------------------*/
void CBasePlayer::DamageEffect(float flDamage, int fDamageType)
{
	...
	else if (fDamageType & DMG_PLASMA)
	{
		...
		// Pequeña sacudida de la pantalla.
...
/*---------------------------------------------------
	...
	Calcular si se debe tomar daño en el modo "Comentarios del desarrollador"
---------------------------------------------------*/
bool CBasePlayer::ShouldTakeDamageInCommentaryMode( const CTakeDamageInfo &inputInfo )
{
	// Al parecer no esta escuchando ningún comentario, activar todo daño.
	...
	// Ignorar todos los tipos de daños excepto: Quemaduras, Plasma, Caidas y Aplastados
...
int CBasePlayer::OnTakeDamage(const CTakeDamageInfo &inputInfo)
{
	...
	// Modo Dios - ¿Que hace dios teniendo daño?
	...
	// Al parecer estamos en un vehículo
	if (pVehicle)
	{
	...
	// Tenemos traje de protección
	// NOTA: El traje de protección no te ayuda cuando estas: Cayendo, Ahogandote, Envenenado o en una zona de radiación
	...
	// Mostrar cualquier efecto relacionado al daño
	DamageEffect(info.GetDamage(), bitsDamage);

	// Calcular el daño producido
	// Sonidos del traje de protección: ¿Qué tan grave es el daño doctor?
	...
	while ((!ftrivial || g_pGameRules->Damage_IsTimeBased(bitsDamage)) && ffound && bitsDamage)
	{
		...
		{
			if (fmajor)
				SetSuitUpdate("!HEV_DMG4", false, SUIT_NEXT_IN_30SEC); // Fractura pequeña
		...
		{
			if (fmajor)
				SetSuitUpdate("!HEV_DMG5", false, SUIT_NEXT_IN_30SEC); // Fractura pequeña
			else
		...
		if (bitsDamage & DMG_SLASH)
		{
			if (fmajor)
				...
				SetSuitUpdate("!HEV_DMG0", false, SUIT_NEXT_IN_30SEC); // Laceraciones pequeñas
		...
		{
			SetSuitUpdate("!HEV_DET2", false, SUIT_NEXT_IN_1MIN); // Niveles peligrosos de radiación.
		...
		{
			SetSuitUpdate("!HEV_SHOCK", false, SUIT_NEXT_IN_5MIN); // Avería en el sistema electrico.
			EmitSound("Player.Pain");
...
void CBasePlayer::OnDamagedByExplosion(const CTakeDamageInfo &info)
{
	...
	// Calcular la distancia de la explosión y ver si es adecuado hacer el sonido de zumbido.
	bool ear_ringing		= (distanceFromPlayer < MIN_EAR_RINGING_DISTANCE) ? true : false;
	// Calcular si el daño causado merece darle al jugador el efecto de confusión.
...
void CBasePlayer::RemoveAllItems( bool removeSuit )
{
	...
	// Quitar también el traje de protección.
...
int CBasePlayer::OnTakeDamage_Alive(const CTakeDamageInfo &info)
{
	...
	// Enviar evento ¡me he dañado!
	IGameEvent * event = gameeventmanager->CreateEvent("player_hurt");

	// Enviar más información al evento. Multiplayer?
	if (event)
	{
...
void CBasePlayer::Event_Killed(const CTakeDamageInfo &info)
{
	...
	// Detener la vibración del control (En caso de estar en una consola)
	...
	// Resetear traje de protección y FOV.
...
/*---------------------------------------------------
	SetAnimation()
	Ajustar animación del jugador o usuario.
---------------------------------------------------*/
...
bool CBasePlayer::CanPickupObject( CBaseEntity *pObject, float massLimit, float sizeLimit )
{
		...
		if (checkEnable)
		{
			...
			// Permitir si es un objeto con físicas o esta establecido que se puede recojer
...
/*---------------------------------------------------
	...
	Calcular y checar el tiempo de daño.
---------------------------------------------------*/
...
/*
	EL TRAJE DE PROTECCIÓN - INMARK 1.0

	El traje de protección es originalmente creado por Valve para Half-Life y Half-Life 2.
	En la siguiente descripción se explica el uso y las funciones del traje de protección
	de siempre así como el "nuevo" traje de protección desarrollado para Apocalypse.

	------------------------------------------------------------------------------------------

	El traje de protección proporciona 2 funciones principales: PROTECCIÓN Y NOTIFICACIÓN.
	Algunas de estas funciones son activadas automáticamente, otras requieren energía.
	El traje de protección permanece con el usuario en todo el juego.
	...
	Radiación
		El usuario podrá ser inmune a los efectos de la radiación.
		NOTA: Excepto cuando el usuario cae en aguas radioactivas.

	Envenenamiento
		Podrá curar al usuario cuando este sea envenando por alguno de sus enemigos.

	Salud
		Podrá curar al usuario cuando este adquiera botiquines o en periodos de tiempo largos.

	Inmunidad
		El usuario podrá ser inmune a cualquier daño durante unos segundos.
		Cabe mencionar que esta función usará la energía del traje para poder ser activada.

	Aumento de poder
		Las armas del usuario podrán aumentar su daño durante unos segundos.
		Cabe mencionar que esta función usará la energía del traje para poder ser activada.

	Armadura
		La armadura protegerá al usuario de impactos, balas y explosiones, esto quiere decir
		que cierto porcentaje del daño será anulado.
		Cabe mencionar que esta función usará la energía del traje para poder ser activada.

	Reanimación
		Podrá revivir al usuario dependiendo de la situación.
...
/*---------------------------------------------------
	CheckSuitUpdate()
	Verifica si hay actualizaciones a aplicar en el traje de protección.
	NOTA: Más que nada sentencias a reproducir.
---------------------------------------------------*/
void CBasePlayer::CheckSuitUpdate()
{
	...
	// Actualizar el contador Geiger en caso de estar en una zona de radiación.
	UpdateGeigerCounter();
...
/*---------------------------------------------------
	SetSuitUpdate()
	Reproduce una setencia del traje de protección.
---------------------------------------------------*/
...
/*---------------------------------------------------
	UpdatePlayerSound()
	Actualiza la posición del slot de sonido del jugador.
---------------------------------------------------*/
...
/*---------------------------------------------------
	...
	Solución "temporal" para calcular el espacio libre
...
/*---------------------------------------------------
	PreThink()
	Ejecución de tareas del jugador.
---------------------------------------------------*/
...
void CBasePlayer::PostThink()
{
	...
	if (!g_fGameOver && !m_iPlayerLocked)
	{
		...
		if (IsAlive())
		{
			...
			// Seleccionar la animación apropiada para el jugador.
...
/*---------------------------------------------------
	FindPlayerStart()
	Encontrar la entidad donde el jugador comenzará.
...
CBaseEntity *CBasePlayer::EntSelectSpawnPoint()
{
	...
	else if (g_pGameRules->IsDeathmatch())
	{
		...
		// Si no se encontro un lugar válido para hacer Spawn, encontrar un lugar adecuado automáticamente.
		if (pSpot)
		{
...
/*---------------------------------------------------
	InitialSpawn()
	El jugador será "spawneado" por primera vez.
---------------------------------------------------*/
...
void CBasePlayer::Spawn( void )
{
	...
	// Resetear variables para detección del agachado.
	...
	// Si el jugador ha sido bloqueado, pues seguirá bloqueado
	if (m_iPlayerLocked)
	{
...
{
	// Al parecer no estamos en ningún vehiculo.
...
/*---------------------------------------------------
	...
	Útil para mantener el orden en una grabación.
---------------------------------------------------*/
