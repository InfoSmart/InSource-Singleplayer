...
static ConVar cam_ots_offset("cam_ots_offset",			"20 -50 3",		FCVAR_ARCHIVE);
...
static ConVar cam_ots_translucencythreshold("cam_ots_translucencyThreshold", "20.0", FCVAR_ARCHIVE);
...
void ClientModeShared::OverrideView(CViewSetup *pSetup)
{
	...
	if( ::input->CAM_IsThirdPerson() )
	{
		if ( pPlayer->AllowOvertheShoulderView() )
		{
			...
			float idealcamShoulderOffset[3]			= { 20, -50, -3 }; // ideal local offset; right, fwd, up
