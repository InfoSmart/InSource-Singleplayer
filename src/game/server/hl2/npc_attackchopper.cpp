...
ConVar in_helicopter_npcmodel("in_helicopter_npcmodel", "models/player/ct_urban.mdl", FCVAR_REPLICATED, "Define el modelo de los NPC aventados por daño");
...
void CNPC_AttackHelicopter::Precache( void )
{
	...
	if ( m_bNonCombat == false )
	{
		...
		PrecacheModel(in_helicopter_npcmodel.GetString());
	}
...
void CNPC_AttackHelicopter::DropCorpse( int nDamage )
{
	...
	CBaseEntity *pGib = CreateRagGib(in_helicopter_npcmodel.GetString(), GetAbsOrigin(), GetAbsAngles(), vecForceVector );
	if ( pGib )
	{
