...
ConVar r_flashlightsavaibles("r_flashlightsavaibles", "8", FCVAR_REPLICATED, "Luces de tipo env_projectedtexture disponibles.");
...
bool CClientShadowMgr::Init()
{
	...
	m_nMaxDepthTextureShadows = r_flashlightsavaibles.GetInt();	// Just one shadow depth texture in games, more in tools
...
void CClientShadowMgr::InitDepthTextureShadows()
{
	...
	timer.End();
	Msg("InitDepthTextureShadows took %.2f msec\n", timer.GetDuration().GetMillisecondsF());
}
