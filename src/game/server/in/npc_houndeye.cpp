...
enum 
{
	TASK_CLOSE_EYE = LAST_SHARED_TASK,
	TASK_OPEN_EYE,
	TASK_THREAT_DISPLAY,
	TASK_FALL_ASLEEP,
	TASK_WAKE_UP,
	...
	TASK_GET_PATH_TO_CIRCLE,
	TASK_REVERSE_STRAFE_DIR,
};
...
enum Houndeye_Conds
{
	COND_GROUP_ATTACK = LAST_SHARED_CONDITION,
	COND_GROUP_RETREAT,
	COND_GROUP_RALLEY,
};
...
enum
{
	SCHED_AGITATED = LAST_SHARED_SCHEDULE,
	SCHED_HOP_RETREAT,

	SCHED_ATTACK_STRAFE,
	SCHED_ATTACK_STRAFE_REVERSE,
	SCHED_GROUP_ATTACK,
	SCHED_GROUP_RETREAT,
	SCHED_COVER_WAIT,
	SCHED_GROUP_RALLEY,
...
void CNPC_Houndeye::InitCustomSchedules(void) 
{
	...
	ADD_CUSTOM_TASK(CNPC_Houndeye,	TASK_CLOSE_EYE);
	ADD_CUSTOM_TASK(CNPC_Houndeye,	TASK_OPEN_EYE);
	ADD_CUSTOM_TASK(CNPC_Houndeye,	TASK_THREAT_DISPLAY);
	ADD_CUSTOM_TASK(CNPC_Houndeye,	TASK_FALL_ASLEEP);
	ADD_CUSTOM_TASK(CNPC_Houndeye,	TASK_WAKE_UP);
	ADD_CUSTOM_TASK(CNPC_Houndeye,	TASK_HOP_BACK);

	ADD_CUSTOM_TASK(CNPC_Houndeye,	TASK_GET_PATH_TO_CIRCLE);
	ADD_CUSTOM_TASK(CNPC_Houndeye,	TASK_REVERSE_STRAFE_DIR);
	
	ADD_CUSTOM_CONDITION(CNPC_Houndeye,	COND_GROUP_ATTACK);
	ADD_CUSTOM_CONDITION(CNPC_Houndeye,	COND_GROUP_RETREAT);

	ADD_CUSTOM_SCHEDULE(CNPC_Houndeye,	SCHED_HOP_RETREAT);
	ADD_CUSTOM_SCHEDULE(CNPC_Houndeye,	SCHED_RANGE_ATTACK1);
	ADD_CUSTOM_SCHEDULE(CNPC_Houndeye,	SCHED_ATTACK_STRAFE);
	ADD_CUSTOM_SCHEDULE(CNPC_Houndeye,	SCHED_ATTACK_STRAFE_REVERSE);
	ADD_CUSTOM_SCHEDULE(CNPC_Houndeye,	SCHED_GROUP_ATTACK);
	ADD_CUSTOM_SCHEDULE(CNPC_Houndeye,	SCHED_GROUP_RETREAT);
	ADD_CUSTOM_SCHEDULE(CNPC_Houndeye,	SCHED_CHASE_ENEMY);
	ADD_CUSTOM_SCHEDULE(CNPC_Houndeye,	SCHED_COVER_WAIT);
	ADD_CUSTOM_SCHEDULE(CNPC_Houndeye,	SCHED_GROUP_RALLEY);
	
	ADD_CUSTOM_ACTIVITY(CNPC_Houndeye,	ACT_GUARD);
...
bool CNPC_Houndeye::IsAnyoneInSquadAttacking( void )
{
	...
	{
		/*if (pSquadMember->IsCurSchedule(SCHEDS_RANGE_ATTACK1))
		{
			return true;
		}*/
	}
	...
	case NPC_STATE_COMBAT:
		{
			...
			if ( HasCondition( COND_GROUP_ATTACK ))
			{
			...
			if ( HasCondition( COND_GROUP_RETREAT ))
			{
				return SCHED_GROUP_RETREAT;
			}
			...
			if ( HasCondition( COND_GROUP_RALLEY ))
			{
				return SCHED_GROUP_RALLEY;
			}
