...
// Última vez que fue activado la vista IronSighted.
...
ConVar   in_ironsight_test_offset_z("in_ironsight_test_offset_z", "0");
 
ConVar   in_ironsight_test_ori_offset_x("in_ironsight_test_ori_offset_x", "0");
...
void IronSightTestOffset(ConVar *pConVar, char *pszString)
{
   ...
   // Ajustar valores.
   in_ironsight_test_offset_x.SetValue(pWeapon->GetWpnData().m_expOffset.x);
   in_ironsight_test_offset_y.SetValue(pWeapon->GetWpnData().m_expOffset.y);
   in_ironsight_test_offset_z.SetValue(pWeapon->GetWpnData().m_expOffset.z);
 
   in_ironsight_test_ori_offset_x.SetValue(pWeapon->GetWpnData().m_expOriOffset.x);
   in_ironsight_test_ori_offset_y.SetValue(pWeapon->GetWpnData().m_expOriOffset.y);
...
ConCommand in_ironsight("in_ironsight", IronSightToggle, "Alterna la vista IronSight");

/*----------------------------------------------------
	CalcIronsightsOffsets()
	Calcular y ajustar los angulos para la vista
	IronSight.
----------------------------------------------------*/
void CalcIronsightsOffsets(CBasePlayer *owner, Vector &pos, QAngle &ang)
{
   Vector   forward, right, up, offset;
 
   // Estamos en modo de prueba.
   if (in_ironsight_test_offset.GetBool())
   {
      ang.x      += in_ironsight_test_ori_offset_x.GetFloat();
      ang.y      += in_ironsight_test_ori_offset_y.GetFloat();
      ang.z      += in_ironsight_test_ori_offset_z.GetFloat();

      offset.Init(in_ironsight_test_offset_x.GetFloat(),
                  in_ironsight_test_offset_y.GetFloat(),
                  in_ironsight_test_offset_z.GetFloat());
   }
   else
   {
   ...
   // Obtener la dirección de los ojos.
...
/*---------------------------------------------------
	CalcViewModelView()
	Calcular y ajustar la vista del jugador.
---------------------------------------------------*/
void CBaseViewModel::CalcViewModelView(CBasePlayer *owner, const Vector& eyePosition, const QAngle& eyeAngles)
{
		...
		// Calcular y ajustar la vista "Ironsight" del arma.
		CalcIronsightsOffsets(owner, vmorigin, vmangles);
